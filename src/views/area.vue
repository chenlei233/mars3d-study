...
export default {
  ...
  async mounted() {
    ...
    this.json = res.data.data.map(({regionId, regionName, boundary}, index) => {
  ...
  methods: {
    init() {
      ...
      this.geoJsonLayer = new mars3d.layer.GeoJsonLayer({
        data: { type: 'FeatureCollection', features: this.json },
        flyTo: true,
        symbol: (attr, style, feature) => {
          console.log(feature);
          return {
            color: STYLE_LIST[feature.porperties.filterData],
            opacity: 0.6,
            diffHeight: (feature.porperties.filterData || 1) * 1000 
          }
        },
      })
